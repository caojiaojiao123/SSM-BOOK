	...
	window.onload = function() {
		...
		df.onclick = function() {
			...
			if (flag == false) {
				...
				if (flage == true) {//确定
					//拿到请求地址
					var $url = "http://localhost/ssm-book/deleteBook/" + str
							+ "/${pb.pageNow}";
					...
					window.location.href = "http://localhost/ssm-book/showBookHandler/${pb.pageNow}";
		...
		outIds.onclick = function() {
			...
			if (flag == false) {
				...
				if (flag) {//确定
					window.location.href = "http://localhost/ssm-book/outputUserServlet?action=outselect&ids="
							+ str;
				}
			}
		};
			...
			<ul class="nav nav-tabs">
				...
				<li class="dropdown"><a href="#" class="dropdown-toggle"
					...
					<ul class="dropdown-menu dropdown-menu-right" role="menu">
						<li>
							<form action="http://localhost/ssm-book/GaoJiSs"
							...
							<table class="table table-bordered table-hover " cellspacing="0"
								...
								<c:forEach items="${pb.beanList }" var="s" varStatus="ss">
									<tr align='center'>
										...
										<td><a
											href='http://localhost/ssm-book/updateBookUl/${s.id }/${pb.pageNow }'>
			               ...
			               1.页数小于10
			                   2.页数大于10
								...
								<ul class="pagination ">
									...
									<li><a href="http://localhost/ssm-book/showBookHandler/1">首页</a>
									</li>
									<c:if test="${pb.pageNow>1 }">
										<li><a aria-label="Previous"
											href="http://localhost/ssm-book/showBookHandler/${pb.pageNow-1 }"><span
												aria-hidden="ture">上一页</span></a></li>
									</c:if>
			                   ...
			                   2.页数大于10
									...
									<c:forEach begin="${begin }" end="${end }" var="i">
										<c:choose>
											...
											<c:otherwise>
												<li><a
													href="http://localhost/ssm-book/showBookHandler/${i}">${i }</a>
												</li>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									...
									<c:if test="${pb.pageNow<pb.pages }">
										<li><a
											href="http://localhost/ssm-book/showBookHandler/${pb.pageNow+1 }"
											aria-label="Previous"><span aria-hidden="ture">下一页</span></a>
										</li>
									</c:if>
									...
									<li><a
										href="http://localhost/ssm-book/showBookHandler/${pb.pages}">尾页
									</a></li>
									</p>
