  ...
  <td colspan="4"> <button id="deletes"><input type="submit" value="删除"/>删除</button></td>
	...
	<center>
		...
		<c:if test="${pb.pageNow>1 }">
			<a href="bookServlet?action=showbookByPage&pageNow=${pb.pageNow-1 }">上一页</a>
		</c:if>
		&nbsp;
		<!-- 分两种情况：
		         1,如果页数小于10：1，到pages；
		...
		<c:forEach begin="${begin }" end="${end }" var="i">
			<c:choose>
				...
				<c:otherwise>
					<a href="bookServlet?action=showbookByPage&pageNow=${i}">[${i}]</a>
				</c:otherwise>
			</c:choose>
		...
		<c:if test="${pb.pageNow<pb.pages }">
			<a href="bookServlet?action=showbookByPage&pageNow=${pb.pageNow+1 }">下一页</a>
		</c:if>
		&nbsp;&nbsp; <a
			href="bookServlet?action=showbookByPage&pageNow=${pb.pages }">尾页</a>
	</center>
